...
export default function AdminAnalyticsDashboard() {
  ...
  const handleResetStats = async () => {
    if (!window.confirm('Bạn có chắc chắn muốn reset toàn bộ số liệu thống kê về 0 để tính lại từ đầu theo quy tắc mới (chỉ tính khách / IP khác Admin)?')) {
      return;
    }
    try {
      setLoading(true);
      await api.resetAnalyticsStats();
      await fetchStats();
    } catch (err: any) {
      setError(err.message || 'Không thể reset thống kê.');
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-2">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
            Chế độ mới: Tự động lọc bỏ truy cập từ Admin & IP của Admin (Chỉ ghi nhận người dùng khách thực tế).
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={fetchStats}
            disabled={loading}
            className="px-3.5 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading}
            className="px-3.5 py-2 text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
            <span>🗑️</span> Reset dữ liệu về 0
          </button>
        </div>
      </div>
