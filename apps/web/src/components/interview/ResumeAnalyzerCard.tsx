  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-md space-y-6">
      ...
      {!cvResult ? (
        ...
        <div className="bg-slate-950 border border-purple-500/30 rounded-2xl p-6 space-y-5 animate-fade-in">
          {/* Header Info */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-4 gap-3">
            <div>
              <span className="text-[10px] uppercase font-bold tracking-wider text-purple-400">Ứng Viên Nhận Diện</span>
              <h4 className="text-base font-bold text-white">{cvResult.candidateName}</h4>
              <p className="text-xs text-slate-400">{cvResult.title}</p>
            </div>
            <div className="bg-purple-950/60 border border-purple-500/30 px-3 py-1.5 rounded-xl text-right">
              <span className="text-[10px] uppercase font-bold text-purple-300 block">Cấp Độ Ước Tính</span>
              <span className="text-xs font-bold text-emerald-400">{cvResult.experienceLevel}</span>
            </div>
          </div>

          {/* AI Summary */}
          <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 space-y-1">
            <span className="text-[10px] font-bold text-purple-400 uppercase">Tóm Tắt Năng Lực từ AI:</span>
            <p className="text-xs text-slate-300 leading-relaxed">{cvResult.summary}</p>
          </div>

          {/* Detected Skills */}
          <div className="space-y-2">
            <span className="text-[11px] font-bold text-slate-400 uppercase">Kỹ Năng Nhận Diện Được:</span>
            <div className="flex flex-wrap gap-1.5">
              {cvResult.detectedSkills.map((skill, idx) => (
                <span
                  key={idx}
                  className="px-2.5 py-1 text-xs font-semibold bg-purple-500/10 text-purple-300 border border-purple-500/20 rounded-lg"
                >
                  {skill}
                </span>
              ))}
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-bold text-slate-400 uppercase">Công Nghệ Khớp Trong Database Vinterview:</span>
          ...
          <div className="pt-2 flex gap-3">
            ...
            <button
              ...
              className="px-4 py-3 text-xs font-semibold text-slate-400 hover:text-white bg-slate-900 rounded-xl border border-slate-800 cursor-pointer"
            >
              Chọn CV Khác
            </button>
          </div>
